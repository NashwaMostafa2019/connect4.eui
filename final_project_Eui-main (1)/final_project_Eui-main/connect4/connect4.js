  ...
  e.addEventListener("click", () => {
    ...
    if (sum <= 6 && turn % 2 != 0) {
      ...
      document.getElementById(
        `${e.id}r${sum}`
      ).innerHTML = `<img src="./Rectangle.svg" style="transform: rotate(${randomRotation}deg);"/>`;
      turn++;
      check("red");
      ...
      document.getElementById(
        `${e.id}r${sum}`
      ).innerHTML = `<img src="./RectangleYello.svg" style="transform: rotate(${randomRotation}deg);"/>`;
      turn++;
      check("yellow");
